...
import { setUsers, setUsersPaginated, deleteUserById as removeFromReduxUsers, setCurrentUserById } from '../store/slices/users';
...
const ViewUsersPage: React.FC = () => {
  ...
  const [totalPages, setTotoalPages] = useState(users.length / limit);
  ...
  useEffect(() => {
    ...
    setTotoalPages(users.length / limit);
      ...
      <ul className="space-y-2">
        {sortedUsers.map((user) => (
          ...
          >
            ...
            <div>
              <Link to={`/edit-user/${user.id}`} onClick={()=> dispatch(setCurrentUserById(user.id))}>
